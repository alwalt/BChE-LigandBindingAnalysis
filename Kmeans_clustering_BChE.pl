#!/usr/bin/perl
# THIS HAS BEEN EDITED TO ALSO PRINT ENERGY VALUES (NOT USED IN CLUSTERING)
# Kmeans scripts as used to cluster data in
# Sorin & Pande, Biophys J. 88, 2472-2493 (2005).
# Last modified 11-20-2010 by EJS
...
$input = "\nUsage\:  ANAL_cluster_data_v1.pl  [options]\n
	...
	Note: (a) when using the -mult flag, start counting fields from 1 rather than 0
	as perl cannot read \"0\" in from the command line; (b) -mult can be used
...
while(defined($line = <DAT>)) {  
  ...
  $coul[$i] = $mult{$numexp} * @expression[$numexp];
  $lj[$i] = $mult{$numexp + 1} * @expression[$numexp + 1];
  $clj[$i]= $mult{$numexp + 2} * @expression[$numexp + 2];
  $proj[$i] = $mult{$numexp + 3} * @expression[$numexp + 3];   
  $run[$i] = $mult{$numexp + 4} * @expression[$numexp + 4];   
  $clone[$i] = $mult{$numexp + 5} * @expression[$numexp + 5];
  $time[$i] = $mult{$numexp + 6} * @expression[$numexp + 6];
...
for($num=1;$num<=$n;$num++) {
#
#  print STDOUT "ITERATION $num\tAGREE = $agree\n";
#
  if($agree<$conv) {
    for($j=0;$j<$K;$j++) {
      $numdata[$j]=0;
...
print OUT "Clusters \/ Data points \/ P\/R\/C\/T\n";
$del=0;
$delmin1=0;
$delmin2=0;
for($i=1;$i<=$numtest;$i++) {
...
#  printf OUT "%6d %8.2f %8.2f %8.2f ",$cluster[$i],$mindist[$i],$mindist2[$i],$del;
  printf OUT "%6d ",$cluster[$i];
  for($cujo=0;$cujo<$numexp;$cujo++){ 
    printf OUT " %10.3f ",$test[$i][$cujo];  
  } 
  printf OUT "%10.2f %10.2f %10.2f %8d %8d %8d %8d\n",$coul[$i],$lj[$i],$clj[$i],$proj[$i],$run[$i],$clone[$i],$time[$i];
}
# $delmin1/=$numtest;
# $del1 = sqrt($delmin1);
# $delmin2/=$numtest;
